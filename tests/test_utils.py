...
class TestGetArgsRaw:
    def test_returns_after_first_split(self):
        assert (
            utils.get_args_raw(make_message(".cmd  hello   world")) == "hello   world"
        )
...
def test_escape_quotes_handles_both():
    assert (
        utils.escape_quotes('She said "hi" & <left>')
        == "She said &quot;hi&quot; &amp; &lt;left&gt;"
    )
...
class TestPlatformName:
    ...
    def test_docker(self, monkeypatch):
        ...
        assert "Docker" in utils.get_platform_name()
...
def test_auto_requirements_round_trip(tmp_path):
    ...
    utils._save_auto_requirements(
        str(p), {"pillow": "==11.3.0", "wand": "", "foo": ">=1"}
    )
