...
12. [Security decorators](#security-decorators)
...
- Prefix the method with a [security decorator](#security-decorators) unless
...
Apply one of these to every command. They check the dispatcher
permission table before invoking your method.

| Decorator | Allowed callers |
| --------- | --------------- |
| `@loader.owner` | Userbot owner only. Default for sensitive commands. |
| `@loader.sudo` | Owner + sudoers. |
| `@loader.support` | Owner + sudoers + support users. |
| `@loader.group_owner` | Owner of the *current* group/channel. |
| `@loader.group_admin` | Any admin of the current group. |
| `@loader.group_admin_*` | Fine-grained: `add_admins`, `change_info`, `ban_users`, `delete_messages`, `pin_messages`, `invite_users`. |
| `@loader.group_member` | Any member of the current group. |
| `@loader.pm` | Private messages only. |
| `@loader.unrestricted` | Everyone, everywhere. Use for read-only commands. |

Stacking is allowed: `@loader.group_admin_ban_users` & `@loader.pm` together
gate a command to either context.

`@loader.ratelimit` enforces stricter per-user rate limiting. Combine with
