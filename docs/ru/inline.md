...
Любой словарь-кнопка выше может нести два опциональных поля:

- `style` — `"primary"` (синяя), `"success"` (зелёная), `"danger"`
  (красная). Любое другое значение логируется и игнорируется.
- `icon_custom_emoji_id` — document-id кастомного эмодзи Telegram,
  отрендерится слева от подписи.

```python
{"text": "Удалить", "callback": self.do_delete, "style": "danger"}
{"text": "Подтвердить", "callback": self.confirm,
