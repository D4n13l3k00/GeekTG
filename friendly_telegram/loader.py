...
def use_fs_for_modules():
    try:
        with open(os.path.join(utils.get_data_dir(), "config.json"), "r") as f:
...
LOADED_MODULES_DIR = os.path.join(utils.get_data_dir(), "loaded_modules")

if not os.path.isdir(LOADED_MODULES_DIR):
    os.makedirs(LOADED_MODULES_DIR, mode=0o755, exist_ok=True)
...
class Modules:
    ...
    def __init__(self, use_inline=True):
        ...
        # Without this, Python 3.11+ may GC them mid-flight and silently
        # swallow the result/exception.
        self._bg_tasks = set()

    def _spawn(self, coro):
        """Schedule a fire-and-forget coroutine and keep a hard reference."""
    ...
    def register_all(self, mods=None):  # skipcq: PYL-W0613
        ...
        for mod in mods:
            try:
                ...
                # ("friendly-telegram.modules.<name>") and third-party module
                # introspection (mod.__module__) stay stable across the rename.
                module_name = (
